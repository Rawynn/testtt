<?php if($products = getProductRelatedProducts($product_id, true)):?>
	...
	<ul class="product-list small" data-from="recommendations" data-campaign-id="<?php echo $campaign_id ?>">
		...
		<?php foreach ($products as $product): ?>
			...
			<?php $count++;?>
			<?php if($count==2): break; endif;?>
		<?php endforeach ?>
	</ul>
<?php elseif ($products = getRecommendedProductsForUser(2,  null, array_keys($add_raport), false, false, array_keys($add_raport))): ?>
	<?php addCampaignViewBox($campaign_id = getSectionCampaignId('recommendations')) ?>
