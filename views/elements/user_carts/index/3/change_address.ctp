...
<a class="address-change btn btn-grey <?php echo $type == 'change-shipping-address' && $current_vat_address == $current_shipping_address ? 'left btn-block' : 'right' ?>" data-type="<?php echo $type ?>-button" data-title="<?php echo h(__('Zmień adres', true)) ?>" data-toggle="modal" href="#<?php echo $type ?>" role="button" title="<?php echo h(__('Zmień adres', true)) ?>">
	<?php if ($type == 'change-shipping-address' && $current_vat_address == $current_shipping_address): ?>
		<?php __('Wybierz adres z książki adresowej') ?>
	<?php else: ?>
		<?php __('Zmień adres') ?>
	<?php endif ?>
</a>
