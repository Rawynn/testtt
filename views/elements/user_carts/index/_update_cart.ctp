<?php
	...
	if (isCartAnyAmountGratisChanged()):
		...
		$array= array();
		foreach(getShippingMethodsList(true, false, true, getDefaultPricesType()) as $idnew => $new):
			if($descshipping = getShippingMethodDescription($idnew)):
				$desc = "<button type='button' class='btn btn-link' data-trigger='click' data-toggle='tooltip' data-html='ture' title='".$descshipping."'><i class='fa fa-question-circle'></i></button>";
			else:
				$desc= '';
			endif;
			foreach(getAllShippingMethods() as $all_shipping_method):
				if($all_shipping_method['ShippingMethod']['id']==$idnew):
					if($day=$all_shipping_method['ShippingMethod']['shipping_time']):
						$times = sprintf(__('Przybliżony czas dostawy %s', true), $day);
						$time = $day==1? $times.__(' dzień', true) : $times.__(' dni', true);
					else:
						$time = '';
					endif;
				endif;
			endforeach;
			$array[$idnew]='<span class="shipping name-shipping">'.$new.'</span><span class="shipping desc-shipping">'.$desc.'</span>';
			
		endforeach;
		$output['shipping']['labels'] = $array;
		...
		$aarray=array();
		foreach(getPaymentMethodsList(true, false, getCurrencyPrecision(), getDefaultPricesType()) as $idpay => $newpay):
		if($descpays = getPaymentMethodDescription($idpay)):
			$descpay = '<button type="button" class="btn btn-link" data-trigger="click" data-toggle="tooltip" data-html="ture" title="'.$descpays.'"><i class="fa fa-question-circle"></i></button>';
		else:
			$descpay= '';
		endif;
		$aarray[$idpay]='<span class="shipping name-shipping">'.$newpay.'</span><span class="shipping desc-shipping">'.$descpay.'</span>';
			
		endforeach;
		$output['payment']['labels'] = $aarray;
		...
		$shipping_method_time = anticipateCartDeliveryTime(true, setting('GLOBAL_ORDER_SEND_DATE_BORDER_TIME'), true);
		...
		$output['costs'] = array(
			...
			array(
				'field' => 'cart-price',
				'value' => getCartSumProductsPrice(getDefaultPricesType())==0? '<span>'.__('Twój koszyk<br/>jest pusty', true).'</span>': showPrice(getCartSumProductsPrice(getDefaultPricesType()))
			),
			...
			array(
				...
				'value' => showPrice(getCartSumProductsPrice())
			),
			...
			array(
				...
				'value' => showQuantityValue(getRealProductsCountInCart())
			),
			//Koszt kuponu rabatowego
			array(
				'field' => 'cart-coupon-price',
				'value' => showPrice($coupon_price)
			),
			...
			array(
				...
				'value'  => ($shipping_method_time) ? showDate(date('d-m-Y', strtotime("+".$shipping_method_time." days"))) : '-',
				'toggle' => (int) $shipping_method_time
			),
			//Wartość bonu
			array(
				'field' => 'cart-voucher-price',
				'value' => $voucher ? showPrice($voucher['price']) : ''
			),
