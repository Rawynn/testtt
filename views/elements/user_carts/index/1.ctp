		...
		<div class="payment-section order-section">
			...
			foreach ($__payment_methods as $payment_method_id => $payment_method_name):
				if($descshiping = getPaymentMethodDescription($payment_method_id)):
					$desc = '<button type="button" class="btn btn-link" data-trigger="hover" data-toggle="tooltip" data-html="true" title="'.$descshiping.'"><i class="fa fa-question-circle"></i></button>';
				else:
					$desc= '';
				endif;
