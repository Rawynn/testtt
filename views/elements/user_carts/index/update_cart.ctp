	...
	if (isCartAnyAmountGratisChanged()):
		...
		foreach(getShippingMethodsList(true, false, true, getDefaultPricesType()) as $idnew => $new):
			if($descshipping = getShippingMethodDescription($idnew)):
				$desc = "<button type='button' class='btn btn-link' data-trigger='hover' data-toggle='tooltip' data-html='true' title='".$descshipping."'><i class='fa fa-question-circle'></i></button>";
			else:
				$desc= '';
			endif;
		...
		if($descpays = getPaymentMethodDescription($idpay)):
			$descpay = '<button type="button" class="btn btn-link" data-trigger="hover" data-toggle="tooltip" data-html="true" title="'.$descpays.'"><i class="fa fa-question-circle"></i></button>';
		else:
			$descpay= '';
		endif;
