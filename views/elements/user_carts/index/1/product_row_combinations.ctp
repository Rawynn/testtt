<?php if (module('KITS') && ($kit_id = getProductField($product_id, 'kit_id'))): ?>
			...
			<div class="combination">
				...
				<?php if (!$cart_blocked): ?>
					<a data-toggle="modal" href="#ChangeCombination<?php echo $key ?>" role="button" title="<?php echo h(__('zmień', true)) ?>">
						<?php __('zmień') ?> <i class="fa fa-caret-right"></i>
					</a>
	...
	<?php endif ?>
<?php elseif ($combinations = getProductCombinations($product_id, !checkCanAddNotSellableProductToCart())): ?>
	<div class="combination">
		<ul data-type="product-row-attributes">
			<?php if ($product_possible_combinations = getProductPossibleCombinations($product_id)): ?>
				<?php foreach ($product_possible_combinations[$combination_id] as $attribute): ?>
					<li>
		...
		<?php if (count($combinations) > 1 && !$cart_blocked): ?>
			<a data-toggle="modal" href="#ChangeCombination<?php echo $key ?>" role="button" title="<?php echo h(__('zmień', true)) ?>">
				<?php __('zmień') ?> <i class="fa fa-caret-right"></i>
			</a>
