<tr class="product-row group-options-row group-options-services-<?php echo getServicesList() ? 'true' : 'false' ?>">
	...
	<td colspan="<?php echo $colspan ?>">
		<?php if (userIsSalesrep()): ?>
			<div class="left-options">
				...
				<?php
					$options = array(
						...
						'attributes'        => __('dołącz atrybuty', true),
						'delete'            => __('usuń', true)
					);
				...
				<div class="modal fade" id="CartSetProductAtttibutesModal" tabindex="-1" role="dialog" aria-hidden="true">
					<div class="modal-dialog">
						<div class="modal-content">
							<div class="modal-header">
								<button type="button" class="close" data-dismiss="modal" aria-hidden="true"></button>
							...
							<div class="modal-body">
								<div class="radio">
									<input type="radio" name="data[GroupAction][attributes_type]" checked="checked" value="1" id="GroupActionAttributesType1" data-type="user-cart-change-product-attributes-type"/>
									
									<label for="GroupActionAttributesType1">
										<?php __('dołącz wg ustawień oferty (wszystkie produkty)') ?>
									</label>
								</div>
								
								<div class="radio">
									<input type="radio" name="data[GroupAction][attributes_type]" value="2" id="GroupActionAttributesType2" data-type="user-cart-change-product-attributes-type"/>
									
									<label for="GroupActionAttributesType2">
										<?php __('indywidualnie (zaznaczone produkty)') ?>
									</label>
								</div>
								
								<div class="form-inner">
									<div class="cart-attribues-check-all">
										<span class="clickable" data-type="cart-product-attributes-check-uncheck-all">
											<i class="fa fa-check-square-o"></i> <?php __('Zaznacz / odznacz wszystkie') ?>
										</span>
									</div>
									
									<div class="form-row checkbox-group" data-type="cart-product-attributes">
										<?php
											echo $this->Form->input(
												'ProductAttribute.id',
												array(
							...
							<div class="modal-footer modal-actions">
								...
								<a class="btn-next btn btn-primary btn-lg" data-type="save-user-cart-product-attributes">
									<?php __('Zapisz') ?>
								</a>
							</div>
						</div>
			...
			<div class="right-options">
				...
				<?php if ($edit_offer_mode): ?>
					...
					<?php if (getCartAddProductAutoNumbers()): ?>
						<a class="btn btn-primary btn-form-size" title="<?php echo h(__('Wyłącz automatyczną numerację produktów', true)) ?>" href="<?php echo $this->Html->url(getCartChangeProductAutoNumbersUrl(0)) ?>" <?php echo $cart_blocked ? 'disabled' : '' ?>>
							<?php __('Wyłącz') ?>
						</a>
					<?php else: ?>
						<a class="btn btn-primary btn-form-size" title="<?php echo h(__('Włącz automatyczną numerację produktów', true)) ?>" href="<?php echo $this->Html->url(getCartChangeProductAutoNumbersUrl(1)) ?>" <?php echo $cart_blocked ? 'disabled' : '' ?>>
							<?php __('Włącz') ?> 
						</a>
					<?php endif ?>
				<?php endif ?>
		...
		<?php else: ?>
			<a class="btn btn-grey" data-toggle="modal" href="#DeleteCheckedProducts" data-type="cart-delete-product-button" role="button" title="<?php echo h(__('Usuń zaznaczone', true)) ?>" disabled>
				<span>x</span> <?php __('Usuń zaznaczone') ?>
			</a>
