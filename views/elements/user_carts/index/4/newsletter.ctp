<?php if (module('NEWSLETTER') && $order['Order']['email'] && !getSubscriberInfo($order['Order']['email'])): ?>
	<div class="newsletter-order-section order-section order-section-50">
		<div class="order-section-header">
			<h2>
				<?php __('Newsletter') ?>
			</h2>
		</div>
		
		<div class="order-section-inner">
			<h3>
				<?php echo __('Chcesz otrzymywać newsletter', true).' '.setting('GLOBAL_STORE_NAME').'?' ?>
			</h3>
			
			<?php
				echo $this->Form->create(
					'Subscriber',
					array(
						'url'   => getNewsletterSubscriberAddUrl(),
						'class' => 'ajax-modal-form'
					)
				)
			?>
				<?php
					echo $this->Form->hidden(
						'email',
						array(
							'value' => $order['Order']['email']
						)
					)
				?>
				
				<?php if (($newsletter_groups = getNewsletterGroupsList()) && count($newsletter_groups) > 1): ?>
					<div class="newsletter-groups">
						<?php
							echo $this->Form->input(
								'NewsletterGroup.NewsletterGroup',
								array(
									'div'      => false,
									'label'    => false,
									'type'     => 'select',
									'options'  => $newsletter_groups,
									'multiple' => 'checkbox',
									'default'  => getSuggestedNewsletterGroupsList()
								)
							)
						?>
					</div>
				<?php endif ?>
				
				<?php if (setting('MODULE_NEWSLETTER_CAPTCHA') && !getLoggedUserId()): ?>
					<div class="captcha">
						<div class="g-recaptcha" id="newsletter-after-order-captcha"></div>
					</div>
				<?php endif ?>
				
				<p>
					<input class="btn btn-primary btn-lg" type="submit" value="<?php echo h(__('Dołącz do newslettera', true)) ?>">
				</p>
			<?php echo $this->Form->end() ?>
		</div>
	</div>
<?php endif ?>