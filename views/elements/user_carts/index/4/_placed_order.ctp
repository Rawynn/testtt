...
<div class="row">
	...
	<?php if($order['NewOrderStatusHistory']['comments']):?>
		<tr>
			<td class="hheader"><?php __('Komentarze')?></td>
	...
	<table class="order-summary-table table-flat">
		...
		<?php else: ?>
			<tr>
				...
				<td class="summary-value">
					<span>
						<?php echo getOrderProductsNettoPrice($order['Order']['id']) ?>
					</span>
				</td>
			</tr>
			<tr>
...
<div class="store-data order-section">
	<h2>
		<?php __('Dane sprzedającego') ?>
	</h2>
	
	<div class="order-section-inner">
		<p>
			<strong>
				<?php echo setting('GLOBAL_STORE_COMPANY') ?>
			</strong>
		</p>
		
		<p>
			NIP: <?php echo setting('GLOBAL_STORE_NIP') ?>
		</p>
		
		<p>
			<?php echo setting('GLOBAL_STORE_STREET') ?>
		</p>
		
		<p>
			<?php echo setting('GLOBAL_STORE_POSTCODE').' '.setting('GLOBAL_STORE_CITY') ?>
		</p>
		
		<p>
			<?php echo sprintf(__('Tel.: %s', true), setting('GLOBAL_CONTACT_PHONE_1'))?>
		</p>
		
		<p>
			<?php echo sprintf(__('E-mail: %s', true), setting('GLOBAL_EMAIL_CONTACT') )?>
		</p>
	</div>
</div>