...
<?php elseif ($shipping_method_options): ?>
	<div class="radio shipping-options form form-inline <?php echo count($shipping_method_options) > 1 ? '' : 'hide' ?>" data-type="shipping-options-toggle" data-options-for="<?php echo $shipping_method_id ?>">
		<?php
			echo $this->Form->input(
				'ShippingMethodOption.id',
				array(
					...
					'selected'         => count($shipping_method_options) > 1 ? '' : key($shipping_method_options)
				)
			)
		?>
	</div>
