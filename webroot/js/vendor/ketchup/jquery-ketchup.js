...
(function($) {
  $.ketchup = {
    defaults: {
      ...
      validateEvents      : 'blur.ketchup',                 //the default event when validations get fired on every field
    ...
    initFields: function(form, fields) {
      ...
      valEls.each(function() {
        ...
        for(i = 0; i < vals.length; i++) {
            if(vals[i]['name'] == 'required' || vals[i]['name'] == 'required-textarea' || vals[i]['name'] == 'email') {
                el.parents(self.options.rowClass).addClass("required");
            }
        }
    ...
    fieldsFromForm: function(form) {
      ...
      for(i = 0; i < valEls.length; i++) {
      	form.find(valEls[i]).off(opt.validateEvents);
      	
        form.find(valEls[i]).parents(opt.rowClass).removeClass("required");
      ...
      for(i = 0; i < events.length; i++) {
        el.bind('ketchup.' + events[i], function() {
          ...
          if(tasty != true) {
            ...
            el.parent('.required').addClass('form-ok');
          }
        });
