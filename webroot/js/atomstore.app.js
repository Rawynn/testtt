	...
	attachEvents : function(){
		...
		$("[data-submit=once]").on("submit.form", function(e){
			if ($(this).ketchup("isValid") == null || $(this).ketchup("isValid")){
				$("[type=submit]:not([data-submit=no-disable]), .form-submit, .disable-on-submit", this).addClass("disabled").prop("disabled", true);
			}
		});
		
		$(function() {
    		if ($('.product-listing-selected-filters').length >0){
			    $('.page-desc').remove();
			}
		});
		...
		$('.newsletter-form [type="submit"]').on("click", function(e) {
			...
			var checkboxes = $("[data-type=validate-checkbox]", form);
			if ($("[data-type=validate-checkbox]:checked", form).length != checkboxes.length) {
				e.preventDefault();
				$('.confirm', form).addClass("error");
			}
		});
		
		$(".newsletter-form [data-type=validate-checkbox]").on("change", function(){
			$(this).closest('.confirm').removeClass("error");
		});
		...
		$("[data-gus=true][data-offer=true]").on("keyup.nip", function(){
			var self        = $(this);
			var contact_box = $("a[data-target=OtherContactInfo], a[data-target=OtherContactInfoSave]", $(this).parents("form").get(0));
			
			$.get(
				url_gus_get_raport + "?nip=" + self.val() + "&search_user=1",
				function (json){
					if (Object.keys(json).length > 0){
						...
						for (x in json){
							$("[data-type=user-cart-" + prefix + "-" + x + "-" + id + "]").val(json[x]);
						}
						
						$("[data-type=user-cart-" + prefix + "-username-" + id + "]").val(json.company);
						
						if (contact_box.length == 1){
							var parent = $(contact_box).parent();
		...
		$("[data-type=salesrep-user-select-header]").on("change.user", function(){
			if ($(this).val() == "new"){
				...
				$("[data-type=salesrep-search-user-header]").modal("show");
				
				$("[data-type=salesrep-user-select-header]").val(present_user_id).trigger("update-state");
			}else{
				$("[data-type=salesrep-user-id-header]").val($(this).val());
				
				$("#ChangeSalesrepsClient").on("shown.bs.modal", function(){}).on("show.bs.modal", function(){
		...
		/* Aktywacja przycisku do aktywacji wyszukanego klienta */
		$("[data-type=salesreps-find-user-id-container] [data-type=autocomplete]").on("select-user", function(){
			$("[data-type=salesrep-user-id-find-user-submit-button]").prop("disabled", false);
		});
		...
		if (App.getSetting("webpush") && $("#WebPushAddSubscription").length > 0){
			/* WebPush - odmowa */
			$("[data-type=webpush-refuse]").unbind("click").on("click", function(){
				var date = new Date();
				
				date.setTime(date.getTime() + (1000 * 60 * 60 * 24 * 365));
				
				$.cookie("webPushDenied", true, {expires: date, secure: window.location.protocol == "https:" ? 1 : 0});
				
				$("#WebPushAddSubscription").modal("hide");
			...
			/* WebPush - akceptacja */
			$("[data-type=webpush-confirm").unbind("click").on("click", function(){
				webpush_set_tag("wszyscy");
				
				if (App.getSetting("user-email") != ""){
					webpush_set_tag(App.getSetting("user-email"));
				}
			...
			if(Notification.permission === "default"){
				$("#WebPushAddSubscription").modal("show");
			}
		}
	...
	initComponents: function(){
		...
		var initAutocomplete = function(context){
			$("[data-ac=true]").each(function(index, element){
				...
				if ($(element).attr("data-add-product") == "true"){
					acInst._renderItem = function(ul, item){
						return $("<div>", {"data-type": "product-item"}).html(item.label).appendTo(ul);
					};
					
					$(element).on("autocompleteopen", function(event, ui){
						$("[data-type=user-cart-add-product-id-container] [data-type=products-list]").empty();
						
						var ul = $("<ul>", {
							"class": "product-list gallery col-5"
						});
						
						var added_products = new Array();
						
						$("[data-type=cart-add-product-products-list] [data-type=added-product-row]").each(function(){
							added_products.push($(this).attr("data-product-id"));
						});
						
						$("[data-type=user-cart-add-product-id-container] .ui-autocomplete [data-type=product-item] .product-box").each(function(){
							/* Skopiowanie */
							...
							var a = $("<a>", {
								...
								"data-has-combinations": $(this).attr("data-has-combinations")
							});
							
							var clone = $(this).clone();
							
							if ($(this).attr("data-has-combinations") == "0"){
								for (x in added_products){
									if (added_products[x] == product_id){
							...
							a.on("click", function(){
								if (!$(".product-box", this).hasClass("green") || $(this).attr("data-has-combinations") == "1"){
									if ($(this).attr("data-has-combinations") == "0"){
										$(".product-box", this).addClass("green");
									}
									
									$("[data-type=user-cart-add-product-id]").val($(this).attr("data-product-id"));
									$("[data-type=user-cart-add-product-button]").click();
								}else{
									$(".product-box", this).removeClass("green");
									
									$("[data-type=cart-add-product-products-list] [data-type=added-product-row][data-product-id=" + $(this).attr("data-product-id") + "]").each(function(){
										$("[data-type=delete-offer-tmp-product]", this).click();
									});
								}
						...
						ul.appendTo($("[data-type=user-cart-add-product-id-container] [data-type=products-list]"));
						
						$("[data-type=user-cart-add-product-id-container] [data-type=products-list]").on("scroll", function(){
							ImageLoader.load();
						});
					});
				}
		...
		if ( $( ".button-holder" ).length ){
			var elementheight = $('.button-holder').outerHeight();
			var elementOffset = $('.button-holder').offset().top;
			var scrollLast = 0;
		}
		...
		$(window).scroll(function(e){
			...
			if (scrollTop < scrollLast){
				var distance      = (elementOffset - scrollTop - windowheight + elementheight + 86);
			}else{
				var distance      = (elementOffset - scrollTop - windowheight + elementheight - 35);
			}
			
			if (distance > 0){
				$('.button-holder').addClass('test');
			}else{
				$('.button-holder').removeClass('test');
			}
		...
		function hasScrolled(){
			...
			if (st > lastScrollTop){
				// Scroll Down
				//$('.header-mobile').removeClass('nav-down on-up').addClass('nav-up');
				$('.button-holder').removeClass('on-up').css('bottom', '0px');
			}else{
			// Scroll Up
				if(st + $(window).height() < $(document).height()) {
					if($(window).scrollTop() == 0 || $(window).scrollTop() == $(document).height()- $(window).height()) {
						//$('.header-mobile').removeClass('nav-up').addClass('nav-down on-up');
						$('.button-holder').addClass('on-up');
					}
					else{
						//$('.header-mobile').removeClass('nav-up').addClass('nav-down');
						$('.button-holder').css('bottom', '0');
					}
				}
			}
			lastScrollTop = st;
		...
		switch(this.getAppPath()){
			...
			case "users/add":
				...
				/* Pobranie danych z GUS */
				$("[data-field=nip][data-gus=true]").on("change.nip", function(){
					var self = $(this);
					...
					$.get(
						...
						function (json){
							...
							for (x in json){
//								if ($("[data-field=" + x + "][data-prefix=" + prefix + "]").val() == ""){
									$("[data-type=" + x + "][data-prefix=" + prefix + "]").val();
									$("[data-field=" + x + "][data-prefix=" + prefix + "]").val(json[x]);
//								}
							}
						}
					);
