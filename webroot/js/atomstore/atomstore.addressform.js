...
function changePhonePrefix(countryValue, inputPhone){
    switch(countryValue) {
        case '2':
            inputPhone.val("+420");
            break;
...
AddressForm.prototype.toggleSelectedUserType = function(value){
	...
	switch (value){
		/* Osoba prywatna */
		case "p":
			$("[data-type=company-row], [data-type=nip-row], [data-type=dic-row]", this.options.container).addClass("hide");
		...
		/* Firma */
		case "f":
			$("[data-type=company-row], [data-type=nip-row], [data-type=dic-row]", this.options.container).removeClass("hide");
