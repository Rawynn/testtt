var Cart = (function(){
	...
	function attachEvents(){
		...
		/* Zmiana kombinacji */
		$("[data-type=change-combination-link]").on("click.cart", function(){
			var key = $(this).attr("data-key");
			
			if ($("#ChangeCombination" + key).length == 0){
				$.get(
					App.getAjaxUrl(url_user_carts_change_combination + "/" + key + "?show=modal"),
					function (response){
						$("[data-type=product-combination-box][data-key=" + key + "] input:hidden").remove();
						
						$("[data-type=product-combination-box][data-key=" + key + "]").append(response);
						
						$("[data-type=change-combination][data-key=" + key + "]").on("change.cart", function(){
							initCurrentRow(this);
				...
				$("#ChangeCombination" + key).modal("show");
			}
		...
		$("[data-type=user-cart-add-product-submit]").on("click", function(){
			...
			$("[data-type=user-cart-add-product-form] input:hidden, [data-type=user-cart-add-product-form] select").each(function(){
				var self = $(this);
		...
		if (App.getSetting("offer-edit-mode")){
			$("[data-type=product-row-image]").on("click", function(){
				if ($(this).attr("data-kit-id") == "0"){
					$.get(
						App.getAjaxUrl(url_user_carts_edit_image + "/" + $(this).attr("data-product-key") + "?show=modal"),
						function (response){
		...
		/* Usuwanie produktu */
		$("[data-type=delete-product]").on("click.cart", function(){
			$.get(
				App.getAjaxUrl(url_user_carts_delete + "?key=" + $(this).attr("data-key") + "&show=modal"),
				function (response){
					$(response).on("shown.bs.modal", function(){
						$(document.body).addClass("modal-open");
					}).on("hidden.bs.modal", function(){
		...
		$("[data-type=cart-group-actions-select]").on("change.group_action", function(){
			...
			if ($(this).val() != ""){
				if ($(this).val() == "offer_union"){
					...
					$("[data-type=cart-group-actions-submit]").addClass("hide");
				}else if ($(this).val() == "attributes"){
					$("#CartSetProductAtttibutesModal").modal("show");
		...
		/* Zaznaczenie wszystkich atrybutów przy indywidualnym odznaczaniu atrybutów do produktów - #21302 */
		$("[data-type=cart-product-attributes-check-uncheck-all]").on("click", function(){
			var checked = true;
			...
			$("[data-type=cart-product-attributes] input[type=checkbox]").each(function(){
				var self = $(this);
		...
		/* Zmiana typu dołączenia atrybutów do produktów - #21302 */
		$("[data-type=user-cart-change-product-attributes-type]").on("click", function(){
			if ($(this).val() == "1"){
				$("[data-type=cart-product-attributes] input[type=checkbox]").prop("disabled", true).trigger("toggle-disabled");
			}else{
				$("[data-type=cart-product-attributes] input[type=checkbox]").prop("disabled", false).trigger("toggle-disabled");
			}
		});
		...
		/* Aktualizacja atrybutów produktów w ofercie */
		$("[data-type=save-user-cart-product-attributes]").on("click", function(){
			var form = $("<form>", {
				"method": "POST",
				"action": url_user_carts_set_products_attributes
			});
			...
			$("#CartSetProductAtttibutesModal input").each(function(){
				var clone = $(this).clone();
		...
		$("[data-type=edit-product-custom-description]").on("click", function(){
			$.get(
				App.getAjaxUrl(url_user_carts_change_product_custom_description + "/" + $(this).attr("data-key") + "?show=modal"),
				function (response){
					$(response).on("shown.bs.modal", function(){
						$(document.body).addClass("modal-open");
		...
		$("a[data-type=product-name][data-key]").on("click", function(){
			$.get(
				App.getAjaxUrl(url_user_carts_change_product_name + "/" + $(this).attr("data-key") + "?show=modal"),
				function (response){
					$(response).on("show.bs.modal", function(){
						$(document.body).addClass("modal-open");
		...
		/* Ustawienie pozycji produktu w koszyku - #21302 */
		$("[data-type=set-product-position]").on("click", function(){
			$("[data-type=set-product-position-product-key]").val($(this).attr("data-key"));
			
			$("#SetProductPosition").modal("show");
		});
